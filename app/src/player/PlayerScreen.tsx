...
function OpenedClip({
  ...
  /* Which saved loop the player is *on*, and therefore the one a save writes
     ...
     target. Same argument #28 makes for holding the chain as ids.

     The two are deliberately separate: the chain is *which loops to play*, this
     is *which loop a save writes to*. Ticking one into a run is not opening it,
     and a chain of four would have no single answer to give this. */
  ...
  const recallLoop = (entry: SavedLoop) => {
    ...
    setChain([])
    /* And the loop becomes the one a save writes over (#30) — which is the whole
  ...
  const chainLoop = (id: string) => {
    ...
    /* And nothing is open any more (#30 meeting #28). A run is not a loop: four
       loops ticked have no single answer to "which entry does a save write
       over", and leaving the last-opened one pointed at would let the next Save
       quietly replace it with a span covering all four. Released, so a save adds
       the run rather than overwriting one of its parts. */
    setEditingId(null)
