...
19. System **writes the loop** to **SAVED LOOPS** — over the entry the player is
    set to where there is one, appended as a new entry where there is not —
    storing its **name, A, B and speed**; the speed is part of the loop, not a
    separate setting (See BR-09, BR-20)
20. System lists each saved loop with its name and its `A – B · Nx` summary, and
    **marks the one the player is set to** — the entry the next save writes over
    — noting it **unsaved** once it and the player have parted company (See
    BR-20)
21. Dancer recalls a saved loop by tapping it; System restores A, B **and speed**,
    turns looping back on, seeks to A, and that loop becomes the one a save
    writes over
22. Dancer corrects the loop they are on — moving a boundary, changing the tempo
    or typing a new name — and saves again; System **writes over the same entry
    in place**. Where a second loop is wanted out of the same stretch of clip,
    Dancer uses **Save as new** (or **⇧s**) instead (See BR-20)
23. Dancer removes a saved loop with its **×**

### Leaving

24. Dancer returns to the Clips screen via **Clips** in the header
...
| BR-20 | **A save corrects the loop you are on.** The player is *set to* a saved loop from the moment one is opened from the list — and from the moment one is saved, which is the half that is easy to miss: without it the first save of a new loop is still followed by `Loop 2`, `Loop 3`, `Loop 4` as the dancer corrects it. While a loop is open, a save **writes over that entry in place**, keeping its id and its place in the file — the panel lists in start order since #28, so a correction that moves A moves the row by that rule rather than by this one; with none open it appends, as it always did. Adding a second loop out of the same stretch of clip is the deliberate act (**Save as new**, **⇧s**), because correcting the open one is the common move and there is neither a reorder nor an undo to dig out of a list full of near-duplicates. **Settled at US-01-19's build, 2026-09-22, reversing US-01-11's approval gate**, which marked the current entry *by value* — A, B and speed all matching. Value equality let go of the entry at exactly the first drag of a correction, which is when it was needed; identity is the id now, and what value equality used to say by dropping the mark, the entry says for itself (`unsaved`). In `loops.json` this is a **third operation — replace this id** — written as a replacement rather than a removal plus an append so that BR-12's merge still falls out of how small the operations are, and a **no-op where the id has gone**, so a correction can never resurrect a loop the other device deleted. **Ticking loops into a run releases the open one**, because a chain is *which loops to play* and this is *which entry a save writes over*, and four ticked rows have no single answer to the second; tapping a loop, which clears the ticks, opens it again (See Q-05, and Q-06: overwriting is the first way this app can lose a loop, which makes the export more wanted rather than less) |
...
| Q-05 | ~~**Two devices editing loops will clobber each other.**~~ **Resolved — merged, by reading before every write.** Settled at US-01-15's approval gate, 2026-09-04, and recorded in full in BR-12. Neither "warn" nor "accept for v1" was needed, because the merge turned out to cost almost nothing: the only operations are *append this loop*, *drop this id* and — since BR-20, 2026-09-22 — *replace this id*, so re-reading the file and replaying the one change onto what Drive currently holds is a complete merge with no tombstones and no resurrection of deleted loops. The third was written as a replacement rather than a removal plus an append precisely so it would stay that small, and it does nothing where the id has already gone. The `version` field the spike watched increment is the guard on the small race that is left. What this deliberately does **not** buy is safety against a corrupted file, which is handled separately by refusing to overwrite one that will not parse | **Resolved** |
| Q-06 | **Loops need an export and a restore.** They are the thing of value in the whole app and they currently exist in exactly one place. A JSON export/import is the cheapest possible insurance against Q-05 and against a Drive mishap. **More wanted since BR-20, 2026-09-22:** overwriting is the first way this app can lose a loop rather than merely fail to add one, and there is no undo behind it | TBD — wanted |
...
flowchart TD
    ...
    Save --> Saved[SAVED LOOPS; the one the player is set to is marked]
    Saved -.->|tap one| Recall[Restores A, B and speed, re-arms looping, seeks to A]
    Recall --> Loop
    Saved -.->|correct it, then s or Update| Save
    Saved -.->|Save as new or ⇧s| Save
    Saved -.->|×| Saved
